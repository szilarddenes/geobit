...
// Development proxy functions that wrap Firebase functions and provide fallbacks
export const verifyAdminAccess = async (data) => {
  try {
    const result = await httpsCallable(functions, 'admin-verifyAdminAccess')(data);
    return result;
  } catch (error) {
    ...
    return { data: { success: false, error: 'Invalid token' } };
  }
};
...
export const getNewsletters = httpsCallable(functions, 'newsletter-getNewsletters');
