...
const LoginForm = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Development fallback - will use this if Firebase function fails
      const devPasswordMatch = password === 'admin123';
      let success = false;
      let token = null;

      try {
        // Try Firebase admin login first
        ...
        console.warn('Firebase function error, using development fallback:', firebaseError);

        // If Firebase login fails, use development fallback
        if (devPasswordMatch) {
          success = true;
          // Generate a simple token for development
          token = 'dev-admin-' + Math.random().toString(36).substring(2, 15);
        }
      }

      if (success && token) {
        // Store the admin token in localStorage
        localStorage.setItem('geobit_admin_token', token);
        onLogin(true);
      } else {
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-md p-8">
      ...
      <div className="bg-blue-50 text-blue-700 p-3 rounded-md mb-4 text-sm">
        <strong>Development Mode:</strong> Use password "admin123" to login.
      </div>
