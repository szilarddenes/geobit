...
const getOpenRouterApiKey = () => {
    var _a;
    return ((_a = functions.config().openrouter) === null || _a === void 0 ? void 0 : _a.api_key) || process.env.OPENROUTER_API_KEY;
};
    ...
    try {
      ...
      Provide 5-10 high-quality, recent results that would be valuable to geoscientists.`;
        ...
        for (const item of jsonContent) {
            ...
            // Add to batch
            batch.set(contentRef, Object.assign(Object.assign({}, item), { searchId: searchResultsRef.id, createdAt: admin.firestore.FieldValue.serverTimestamp(), status: 'pending' // can be: pending, approved, rejected
             }));
        }
...
const collectContentFromSources = functions.https.onCall(async (data, context) => {
    try {
        ...
        for (const id of sourceIds) {
            ...
            if (sourceDoc.exists) {
                sources.push(Object.assign({ id: sourceDoc.id }, sourceDoc.data()));
            }
        }
        if (sources.length === 0) {
        ...
        for (const item of contentItems) {
            const contentRef = db.collection('content').doc();
            batch.set(contentRef, Object.assign(Object.assign({}, item), { collectionId: collectionRef.id, createdAt: admin.firestore.FieldValue.serverTimestamp(), status: 'pending' // pending, approved, rejected
             }));
        }
...
const getContent = functions.https.onCall(async (data, context) => {
    try {
        ...
        const content = contentSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
